...
## 2026-07-24 - Gabrielle migre vers sa propre API (échange avec Raphaël)

Gabrielle n'appelle plus directement llama.cpp (`POST /v1/chat/completions`, format OpenAI) : elle a désormais sa propre API (`POST /api/v1/ask`, `GABRIELLE_API_URL`, défaut `http://127.0.0.1:8082`), qui gère elle-même l'historique par `session` et interroge **Raphaël** (base de connaissance perso) pour les réponses sourcées. `lib/llamaCpp.ts` remplacé par `lib/gabrielle.ts` ; `app/api/chat/route.ts` n'envoie plus que `{message, session}` (le fil complet n'est plus renvoyé à chaque tour, géré côté Gabrielle, borné à 20 tours / 1h de TTL).

Contrat de réponse : `{reponse, statut, verdict, sources, ts}`, discriminant sur `statut` (`sources` | `conversation` | `indisponible` - `aucune_source` existe dans le contrat mais n'est pas émis en v1, traité comme `conversation` côté rendu). Un Raphaël injoignable ne casse pas la requête : la réponse reste 200 avec `statut: "indisponible"`, pas une erreur HTTP. `verdict` est de l'observabilité pure - loggé en `console.debug` côté serveur, jamais renvoyé au navigateur.

`components/ChatWidget.tsx` : `session` généré côté client (`crypto.randomUUID()`, `sessionStorage` - éphémère par onglet, pas `localStorage`) et transmis tel quel. Rendu par statut : `sources` → petit texte + cartes (provenance en titre, contenu en corps, score en infobulle) ; `conversation`/`aucune_source` → bulle simple ; `indisponible` → réutilise le bloc d'erreur existant avec un message dédié, plutôt qu'un nouveau tour dans le fil.

`lib/chatPrompt.ts` (`GABRIELLE_SYSTEM_PROMPT`) supprimé : le prompt persona vit désormais dans l'API de Gabrielle elle-même, plus dans ce dépôt.

## 2026-07-24 - GABRIELLE_API_URL en conteneur : 127.0.0.1 ne marche pas
...
## 2026-08-03 - AlicIA et le trio Gabrielle/Raphaël/Mickaël : correction d'une confusion de fond

Erreur relevée par Vincent : `docs/architecture.md` et `/labia` présentaient encore Gabrielle comme "le rôle d'accueil que joue AlicIA" (framing d'origine, cf. décision du 2026-07-10 "Chatbot renommé Gabrielle"). Ce n'est plus le cas - AlicIA et le trio sont deux systèmes **totalement séparés**, sans lien technique. Autre inversion corrigée au passage : c'est **Mickaël** qui oriente/décide en interne (mandat de routage), pas Gabrielle - elle reste en coulisses, jamais visible du visiteur.

Corrections : section "Gabrielle vs AlicIA" de `docs/architecture.md` retirée (remplacée par une simple précision d'indépendance) ; paragraphe Mickaël inversé (elle oriente, pas Gabrielle). Sur `/labia`, la phrase qui liait Gabrielle à AlicIA retirée du paragraphe d'intro d'AlicIA, et le trio sorti dans sa propre section ("Gabrielle, Raphaël, Mickaël") plutôt que niché dans "AlicIA, mon projet principal" - avec une explication du pourquoi de la séparation : AlicIA est le projet le plus avancé (autonomie, outils, accès réel), le trio une implémentation plus modeste mais plus maîtrisée (le code décide, jamais le modèle) - c'est cette maîtrise qui permet de l'exposer directement sur le site, contrairement à AlicIA.
...
## 2026-08-04 - `/infra` : section "Déploiement et résilience" + retrait du compte exact de machines

Vincent ne voulait plus donner de compte exact sur `/infra` ("Quatre rôles, quatre machines") - devenu de toute façon inexact depuis l'entrée de l'infra distribuée (VPS + serveur local + Kali, cf. décision du 2026-08-03), la section "Vue d'ensemble" ne listant que trois des machines réellement impliquées. Phrase reformulée en "Plusieurs rôles, plusieurs machines, chacune avec un usage précis :", sans toucher à la liste elle-même.

Nouvelle section "Déploiement et résilience" ajoutée après "Pourquoi ces choix" : reprend au niveau public (générique, cohérent avec la décision "`/infra` allégée" du 2026-07-10) la bascule DNS VPS/serveur local et le pipeline CI/CD (build Docker multi-arch → registre → déploiement SSH) déjà documentés dans le README et `docs/architecture.md`. Un diagramme accompagne la section, rendu avec le package `mermaid` (nouvelle dépendance - aucune lib de rendu Mermaid n'existait déjà dans ce dépôt) via un petit composant client dédié (`components/Mermaid.tsx`, dynamic import + `mermaid.render()`, thème choisi selon `prefers-color-scheme`) : rendu uniquement côté client, pas de rendu serveur pour ce type de diagramme. Le schéma de topologie réseau existant (généré par script, section "Topologie réseau") n'est pas concerné - il documente le homelab local, pas cette infra distribuée.

Vincent a fait remarquer que cette distinction (schéma réseau = homelab local uniquement, page = infra au sens large désormais distribuée) n'était nulle part explicite dans le texte visible - seulement dans le raisonnement interne ci-dessus. Deux ajustements en conséquence : l'intro de la page ("chez moi" retiré, devenu "la porte d'entrée de mon petit système auto-hébergé" - plus vrai depuis que le VPS n'est pas chez lui) et une phrase ajoutée en tête de "Topologie réseau" précisant que ce schéma ne couvre que le réseau local, avec un renvoi vers "Déploiement et résilience" pour la partie distribuée.

## 2026-08-11 - Deuxième tableau de bord public sur `/monitoring` (logs)

Jusqu'ici `/monitoring` n'exposait qu'un seul dashboard Grafana public (métriques, stack PLG). `MonitoringEmbed.tsx` codait cette URL en dur via `NEXT_PUBLIC_MONITORING_EMBED_URL`, ce qui ne permettait qu'un seul embed par page.

`MonitoringEmbed` généralisé : `embedUrl` et `title` sont maintenant passés en props par l'appelant plutôt que lus directement dans le composant, pour pouvoir en instancier plusieurs sur la même page sans dupliquer la logique de placeholder (indisponible / pas encore configuré). `/monitoring` affiche désormais deux sections - "Tableau de bord — Métriques" (inchangé, `NEXT_PUBLIC_MONITORING_EMBED_URL`) et "Tableau de bord — Logs" (nouveau, `NEXT_PUBLIC_MONITORING_LOGS_EMBED_URL`, dashboard public Grafana/Loki curé) - toutes deux soumises au même statut de disponibilité (`getMonitoringStatus()`, même Pi).

Nom de la variable existante conservé tel quel plutôt que renommée en `_METRICS_` par souci de symétrie, pour ne pas casser le `.env` déjà en place sur le Pi sans redeploy coordonné.
